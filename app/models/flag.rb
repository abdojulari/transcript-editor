class Flag < ActiveRecord::Base
  ...
  def self.getByLine(transcript_line_id)
    Flag
      ...
      .joins('INNER JOIN flag_types ON flags.flag_type_id = flag_types.id LEFT OUTER JOIN users ON users.id = transcript_edits.user_id LEFT OUTER JOIN user_roles ON user_roles.id = users.user_role_id')
      .where(transcript_line_id: transcript_line_id, is_deleted: 0, category: 'error')
  end
